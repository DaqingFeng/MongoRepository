...
namespace library
{
    ...
    /// </summary>
    internal static class MongoUtilOld<U>
    {
        /// <summary>
        ...
        private static MongoDatabase GetDatabaseFromUrl(MongoUrl url)
        {
            ...
            return server.GetDatabase(url.DatabaseName, new MongoDatabaseSettings() { GuidRepresentation = GuidRepresentation.Standard }); // WriteConcern defaulted to Acknowledged
        }
        ...
        {
            return MongoUtilOld<U>.GetCollectionFromConnectionString<T>(connectionString, GetCollectionName<T>());
        }
        ...
        public static MongoCollection<T> GetCollectionFromConnectionString<T>(string connectionString, string collectionName)
            where T : IMongoEntity<U>
        {
            return MongoUtilOld<U>.GetDatabaseFromUrl(new MongoUrl(connectionString))
        ...
        public static MongoCollection<T> GetCollectionFromUrl<T>(MongoUrl url)
            where T : IMongoEntity<U>
        {
            return MongoUtilOld<U>.GetCollectionFromUrl<T>(url, GetCollectionName<T>());
        }
        ...
        public static MongoCollection<T> GetCollectionFromUrl<T>(MongoUrl url, string collectionName)
            where T : IMongoEntity<U>
        {
            return MongoUtilOld<U>.GetDatabaseFromUrl(url)
                .GetCollection<T>(collectionName);
        }
